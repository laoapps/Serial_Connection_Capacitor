...
public class SerialConnectionCapacitorPlugin extends Plugin {
    ...
    public void load() {
        ...
        if (sspDevice == null) {
            sspDevice = new SSP();
            Log.d(TAG, "✅ SSP instance pre-created in load()");
        }
    ...
    private void initializeSSPAsync() {
        new Thread(() -> {
            try {
                ...
                if (sspDevice == null) {
                    sspDevice = new SSP();
                    Log.d(TAG, "SSP was null → auto-created in initializeSSPAsync");
                }
            ...
            while (isReading) {
                synchronized (this) {
                    ...
                    try {
                            ...
                            if (len > 0) {
                                ...
                                while (start <= accumulated.length - 5) {
                                    if ((accumulated[start] & 0xFF) == 0xFA && (accumulated[start + 1] & 0xFF) == 0xFB) {
                                        ...
                                        if (start + packetLength > accumulated.length) {
                                            break;
                                        }
                        ...
                        if (isReading) {
                            Log.e(TAG, "VMC read error: " + e.getMessage());
                        }
                    }
                }
            }
            ...
            while (isReading) {
                synchronized (this) {
                    ...
                    try {
                        ...
                        if (isReading) {
                            Log.e(TAG, "Serial read error: " + e.getMessage());
                        }
                    }
                }
            }
    ...
    private int getExpectedResponseLength(String command) {
        switch (command.toUpperCase()) {
            ...
            case "B5":
                return 5;  // RUN2
            case "21":
                return 5;  // switchToTwoWireMode
            case "35":
                return 5;  // setSwap
            default:
                return 4;    // Minimum length
        }
    }
